...
class EmplProfileController extends Controller
{
    ...
    public function getForm(Request $request)
    {
        ...
        $form_htm = view('elements.form', [
            ...
            'tab_id' => $tab_id,
            'tabs_htm' => $this->getFormTabsHTML($tab_id, $params->form_id, $frm_uniq_id, $item_id),
            ...
            'callback_fn' => $request->input('callback', ''),
            'callback_field_id' => $request->input('rel_field_id', ''),
    ...
    public function refreshFormFields(Request $request)
    {
        ...
        return response()->json(['success' => 1, 'html' => $fields_htm]);
    }
    ...
    private function getFormBadge($item_id, $form_is_edit_mode)
    {
        $form_badge = "";
        if ($item_id > 0) {
            $form_badge = ($form_is_edit_mode == 1) ? "Rediģēšana" : "";
        }
        else {
            $form_badge = "Jauns";
        }
    ...
    private function checkSaveRights($form_id, $item_id)
    {
        ...
        if ($right == null) {
            ...
            if ($item_id > 0 && $right->is_edit_rights == 0) {
                throw new Exceptions\DXCustomException("Jums nav ieraksta labošanas tiesības šajā reģistrā!");
            }

            if ($item_id == 0 && $right->is_new_rights == 0) {
                throw new Exceptions\DXCustomException("Jums nav jauna ieraksta veidošanas tiesības šajā reģistrā!");
            }
        }
    }
    ...
    private function getFormFieldsHTML($frm_uniq_id, $list_id, $item_id, $parent_item_id, $parent_field_id, $params)
    {
        ...
        foreach ($fields as $row) {
            ...
            $fields_htm = $fields_htm . $fld_obj->get_field_htm();
     ...
     */
    private function getFormTabsHTML($tab_id, $form_id, $frm_uniq_id, $item_id)
    {
        $tabs_items = $this->getFormTabs($form_id);
        $tabs_htm = "";

        if (count($tabs_items) > 0) {

            $tabs_htm = view('elements.tabs', [
                'tab_id' => $tab_id,
