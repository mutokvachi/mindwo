...
namespace App\Libraries\FieldsSave
{
    ...
    class FieldSave_file extends FieldSave
    {
        ...
        {
            $this->file_folder = $this->folderSlash(\App\Http\Controllers\FileController::getFileFolder($this->fld->is_public_file));
            ...
            foreach ($paths as $path) {                
                ...
                if ($path->width > 0 && $path->height > 0) {                    
                    ...
                    if ($path->is_for_gallery) {
                        Image::resizeGalery($this->file_folder, $target_file, $path->width, $path->height, $this->folderSlash($path->folder_path));
                    }
                    else {
                        Image::resize($this->file_folder, $target_file, $path->width, $path->height, $this->folderSlash($path->folder_path));
                    }                    
                }
                else {                    
                    if (!File::copy($this->file_folder . $target_file, $this->folderSlash($path->folder_path) . $target_file)) {
                        throw new Exceptions\DXCustomException("Sistēmas kļūda! Nav iepsējams kopēt datni '" . $this->file_folder . $target_file . "' uz katalogu '" . $this->folderSlash($path->folder_path) . "'.");
                    }
                }
            }
        }
         ...
         */
        private function folderSlash($folder) {
            $folder = trim($folder);
