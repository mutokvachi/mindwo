...
class Filemanager
{
	...
	public function rename() {
		...
		if(!rename($old_file,$new_file)) {
			if(is_dir($old_file)) {
                ...
                if (!\App\Libraries\ResourcesSave::renameFileOrFolder($old_file, $new_file)) {
                    $this->error("Nav iespējams pārsaukt.");
                }
	...
	public function delete() {
		...
		if(is_dir($current_path)) {
                        ...
                        if (!\App\Libraries\ResourcesSave::isDirEmpy($current_path)) {
                            $this->error("Katalogu nevar dzēst, jo tas satur kādu datni. Vispirms jādzēš datnes.");
                        }
			...
			$this->unlinkRecursive($thumbnail_path);
                        ...
                        if (!\App\Libraries\ResourcesSave::deleteFolder($current_path)) {
                            $this->error("Nav iespējams dzēst katalogu.");
                        }
			...
			if(file_exists($thumbnail_path)) unlink($thumbnail_path);
                        ...
                        if (!\App\Libraries\ResourcesSave::deleteFile($current_path)) {
                            $this->error("Nav iespējams dzēst datni.");
                        }
                        $array = array(
				'Error'=>"",
				'Code'=>0,
		...
		if($this->is_image($imagePath) && $this->config['images']['resize']['enabled']) {
			if ($size = @getimagesize($imagePath)){
				if ($size[0] > $this->config['images']['resize']['maxWidth'] || $size[1] > $this->config['images']['resize']['maxHeight']) {
					...
					$resized = $image->resize($this->config['images']['resize']['maxWidth'], $this->config['images']['resize']['maxHeight'],function ($constraint) {$constraint->aspectRatio();});
	...
	public function add() {
		...
		if($this->is_image($imagePath) && $this->config['images']['resize']['enabled']) {
			if ($size = @getimagesize($imagePath)){
                ...
                if (!\App\Libraries\ResourcesSave::saveFile($imagePath)) {
                    $this->error("Datnes saglabāšanas kļūda.");
                }
	...
	public function addfolder() {
		...
		if(!mkdir($current_path . $newdir,0755)) {
			$this->error(sprintf($this->lang('UNABLE_TO_CREATE_DIRECTORY'),$newdir));
                ...
                if (!\App\Libraries\ResourcesSave::createFolder($current_path . $newdir)) {
                    $this->error("Nav iespējams izveidot katalogu.");
                }
