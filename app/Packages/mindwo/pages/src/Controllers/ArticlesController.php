...
class ArticlesController extends Controller
{
    ...
    public function showTagArticles(Request $request, $tag_id)
    {
        ...
        return view('mindwo/pages::pages.articles', compact('articles', 'page_title', 'criteria', 'mode', 'block_guid', 'types', 'type_id', 'tag_id', 'date_from', 'date_to'));
    }
    ...
    public function searchArticle(Request $request)
    {
        ...
        $d_from = getDateTimeObj($date_from, 1);
        $d_to = getDateTimeObj($date_to, 0);

        $types = $this->getArticlesTypes($d_from, $d_to, $criteria);

        $articles = get_article_query()
                ->whereDate('in_articles.publish_time', '>=', $d_from->format('Y-m-d'))
                ->whereDate('in_articles.publish_time', '<=', $d_to->format('Y-m-d'))
                ->where('in_articles.is_active', '=', 1)
                ->where('in_articles.is_searchable', '=', 1)
                ->where(function ($query) use ($type_id)
                {
        ...
        $mode = 'search';
        $picker_from_js = get_js_for_datetime_field('search_form', 'pick_date_from');
        $picker_to_js = get_js_for_datetime_field('search_form', 'pick_date_to');

        $picker_from_html = get_htm_for_datetime_field('search_form', 'pick_date_from', $date_from);
        $picker_to_html = get_htm_for_datetime_field('search_form', 'pick_date_to', $date_to);
        $block_guid = Uuid::generate(4);

        $tag_id = 0;
        $page_title = "Meklēšanas rezultāti";
        return view('mindwo/pages::pages.articles', compact('articles', 'page_title', 'criteria', 'mode', 'picker_from_js', 'picker_from_html', 'picker_to_js', 'picker_to_html', 'block_guid', 'types', 'type_id', 'tag_id', 'date_from', 'date_to'));
    }
    ...
    private function getTagItemRow($tag_id)
    {
        ...
        if (!$item) {
            throw new PagesException("Iezīme ar id " . $tag_id . " nav atrasta!");
        }
     ...
     * @return Array Rakstu tipu masīvs
     */
    private function getArticlesTypes($d_from, $d_to, $criteria)
    {
        $types = $this->getTypes();
