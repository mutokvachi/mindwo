...
class DxHdInnerTypesUi extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::transaction(function () {
            $table_name = "dx_hd_inner_types";
            $list_name = "Pieteikumu tipi";
            $item_name = "Pieteikuma tips";
            ...
            // make menu
            $parent = DB::table('dx_menu')->where('title', '=', 'Atbalsts')->first();
            
            if ($parent) {
                DB::table('dx_menu')->insertgetId(['parent_id' => $parent->id, 'title'=>$list_name, 'list_id'=>$list_id, 'order_index' => 20]);
            }
    ...
    {
        DB::transaction(function () {
            \App\Libraries\DBHelper::deleteRegister('dx_hd_inner_types');
            DB::table('dx_objects')->where('db_name', '=', 'dx_hd_inner_types')->delete();
        });
    }
}
