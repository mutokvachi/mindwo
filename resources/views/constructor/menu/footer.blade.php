<div class="dx-stick-footer animated bounceInUp">
    <div class='row'>
      <div class='col-lg-2 col-md-3 hidden-sm hidden-xs dx-left dx-stick-footer-title'>
        <div class="row">
          <div class="col-md-12">
            <i class="fa fa-sitemap"></i>
            <span>{{ trans('constructor.menu.title') }}</span>
          </div>
        </div>
      </div>
      <div class='col-lg-10 col-md-9 col-sm-12 col-xs-12 dx-right'>
        <div class="row">
          <div class="col-md-12">
            <a href="javascript:;" class="btn btn-default dx-new-btn">
              <i class="fa fa-plus"></i> {{ trans('constructor.menu.new_btn') }}
            </a>
            <a href="javascript:;" class="btn btn-default dx-undo-btn" style="display: none;">
              <i class="fa fa-undo"></i> {{ trans('constructor.menu.undo_btn') }}
            </a>
            <a href="javascript:;" class="btn btn-primary dx-save-btn">
              <i class="fa fa-save"></i> {{ trans('constructor.menu.save_btn') }}
            </a>
          </div>
        </div>
      </div>
    </div>
</div>