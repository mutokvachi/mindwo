    ...
    <script id="template-upload" type="text/x-tmpl"> {% for (var i=0, file; file=o.files[i]; i++) { %}
          <tr class="template-upload fade">
              ...
              <td class="error" width="20%" colspan="2">
                  <span class="label label-danger">{{ trans('mail.error') }}</span> {%=file.error%}</td> {% } else if (o.files.valid && !i) { %}
              <td>
    ...
    <script id="template-download" type="text/x-tmpl"> {% for (var i=0, file; file=o.files[i]; i++) { %}
          <tr class="template-download fade"> {% if (file.error) { %}
              ...
              <td class="error" width="30%" colspan="2">
                  <span class="label label-danger">{{ trans('mail.error') }}</span> {%=file.error%}</td> {% } else { %}
              <td class="name" width="30%">
