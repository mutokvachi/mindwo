...
function rel_new_form(ajax_url, list_id, item_id, rel_field_id, callback)
{
        var formData = new FormData();
        formData.append("item_id", item_id);
        formData.append("list_id", list_id);
...
function refresh_form_fields(edit_form_htm_id, form_htm_id, item_id, list_id, rel_field_id, rel_field_value)
{
    ...
    formData.append("frm_uniq_id", $("#" + form_htm_id).find("div[dx_attr=form_fields]").attr("dx_form_id"));
...
{    
    ...
    request.callback = function(data) {
        debug_log("Callback from AJAX save");
        show_form_splash();
        if (grid_htm_id)
        {
            debug_log("Reload grid");
            reload_grid(grid_htm_id);
        }
        
        if (parent_form_htm_id)
        {            
            debug_log("Refresh form data start");
