	...
	function createNode(nodeData, level, opts)
	{
		...
		$nodeDiv.on('click', '.topEdge', function(event)
		{
			...
			else
			{
				...
				if(startLoading($that, $node, opts))
				{
					// load new nodes
					$.ajax({
						'url': $.isFunction(opts.ajaxURL.parent) ? opts.ajaxURL.parent(nodeData) : opts.ajaxURL.parent + nodeId,
						'dataType': 'json'
					})
						.done(function(data)
						{
		...
		$nodeDiv.on('click', '.bottomEdge', function(event)
		{
			...
			{ // load the new children nodes of the specified node by ajax request
				...
				{
					$.ajax({
						'url': $.isFunction(opts.ajaxURL.children) ? opts.ajaxURL.children(nodeData) : opts.ajaxURL.children + nodeId,
						'dataType': 'json'
					})
						.done(function(data, textStatus, jqXHR)
						{
